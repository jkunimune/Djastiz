...
- `i` denotes the _indirect object_ of the action, the concept that the subject indirectly affects or references (e.g. the order of `shaid`). It often functions in tandem with `u` (e.g. the old and new values in `ry`). `i` is never applicable when `u` is not.
